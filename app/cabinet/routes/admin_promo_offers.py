...
async def broadcast_offer(
    ...
    # site_notify_targets is ours and covers every recipient, including those
    # with neither telegram_id nor a verified email -- upstream's guard alone
    # would skip the whole block for a site-trial-only audience.
    if payload.send_notification and (notify_targets or email_targets or site_notify_targets):
        ...
        if site_notify_targets:
            # Ours: fan the same promo out to the site notification bell + Web
            # Push. Deliberately its own branch rather than nested under
            # notify_targets -- a recipient with no telegram_id never lands in
            # notify_targets at all, and for an email/site-trial-only user the
            # bell is the only channel that reaches them.
